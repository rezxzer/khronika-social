...
export default function FollowersPage() {
  ...
  const fetchFollowers = useCallback(
    async (offset = 0, append = false) => {
      ...
      if (data) {
        const mapped = data
          .map((r) => r.profiles as unknown as FollowerProfile)
          .filter(Boolean);
  ...
  useEffect(() => {
    if (targetUserId) fetchFollowers(0);
    ...
    <AppShell>
      <div className="mx-auto max-w-lg space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-serif text-xl font-bold">მიმდევრები</h1>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center rounded-xl border border-dashed py-12 text-center text-muted-foreground">
            ...
            <p className="font-medium">ჯერ მიმდევრები არ ჰყავს</p>
          </div>
        ) : (
          <div className="space-y-2">
            ...
            {hasMore && (
              <div className="flex justify-center pt-2">
                <Button
                  variant="outline"
                  onClick={() => fetchFollowers(profiles.length, true)}
